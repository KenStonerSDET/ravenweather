...
AUDIO_DIR=/home/pi/wx-ground-station/audio
IMAGE_DIR=/home/pi/wx-ground-station/images
LOG_DIR=/home/pi/wx-ground-station/logs
...
if [ -e $AUDIO_FILE ]
  then
    ...
    CHAN_B=`grep "Channel B" $LOGFILE | head -1`
     ...
     echo "node /home/pi/wx-ground-station/aws-s3/upload-wx-images.js \"$SAT\" $FREQ ${IMAGE_DIR}/${FILEKEY} $ELEVATION $DIRECTION $DURATION \"${TLE1}\" \"${TLE2}\" \"$GAIN\" \"${CHAN_A}\" \"${CHAN_B}\"" >> $LOGFILE 2>&1
    node /home/pi/wx-ground-station/aws-s3/upload-wx-images.js "$SAT" $FREQ ${IMAGE_DIR}/${FILEKEY} $ELEVATION $DIRECTION $DURATION "${TLE1}" "${TLE2}" "$GAIN" "${CHAN_A}" "${CHAN_B}" >> $LOGFILE 2>&1
fi
