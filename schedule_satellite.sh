...
SAT=$1
FREQ=$2
TLE_FILE=/home/pi/wx-ground-station/weather.tle
...
while [ $END_EPOCH_DATE == `date +%D` ] || [ $END_EPOCH_DATE == `date --date="tomorrow" +%D` ]; do
  ...
  if [ $MAXELEV -ge 20 ]
    then
      FILEKEY="${SAT//" "}-${OUTDATE}"
      COMMAND="/home/pi/wx-ground-station/receive_and_process_satellite.sh \"${SAT}\" $FREQ $FILEKEY $TLE_FILE $START_EPOCH $JOB_TIMER $MAXELEV $DIR"
      echo $COMMAND
      ...
      echo ${START_EPOCH},${END_EPOCH},${MAXELEV},${DIR},${SAT},"${TLE1}","${TLE2}" >>  /home/pi/wx-ground-station/upcoming_passes.txt
  fi
